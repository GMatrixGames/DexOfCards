...
{
    public static async Task<int> AddNewCard(CardModel model)
    {
    ...
    public static async Task<int> RemoveCard(CardModel model)
    {
        var currentAmount = await GetOwnedAmount(model);
    ...
    public static async Task<int> GetOwnedAmount(CardModel model)
    {
        var amount = 0;
